...
/*   Updated: 2022/07/20 08:19:20 by juhur            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap(std::string name, int hitPoint, int energyPoint, int attackDamage)
  ...
  std::cout << "Constructor " << __func__ << "(" << this->name << ", " << this->hitPoint << ", " << this->energyPoint << ", " << this->attackDamage << ") is called\n";
}
...
ClapTrap& ClapTrap::operator=(const ClapTrap& obj) {
  std::cout << "Copy assignment operator(ClapTrap) is called\n";
  this->name = obj.name;
...
void ClapTrap::attack(const std::string& target) {
  if (this->hitPoint == 0) {  // ATTACKER's HP is 0
    std::cout << this->name << "(ClapTrap) is already dead. " << target;
    this->attackDamage = 0;
  } else if (this->energyPoint == 0) {  // ATTACKER's EP is 0
    std::cout << this->name << "(ClapTrap) doesn't have enough energy. " << target;
    this->attackDamage = 0;
  } else {  // ATTACK !!
    std::cout << this->name << "(ClapTrap) attacks " << target;
    --this->energyPoint;
  }
}
...
void ClapTrap::beRepaired(unsigned int amount) {
  ...
  if (beforeHP > this->hitPoint + amount)
    ...
    this->hitPoint += amount;
  --this->energyPoint;
  std::cout << this->name << " HP [" << this->hitPoint - beforeHP << "] UP\n";
}
